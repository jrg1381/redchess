        ...
        public void AddAnalysis(int gameId, int moveNumber, IBoardAnalysis boardAnalysis)
        {
            ...
            using (var board = BoardFactory.CreateInstance())
            {
                ...
                if (boardAnalysis.BoardEvaluationType == EvaluationType.MateInN)
                {
                    ...
                    var matches = rx.Matches(boardAnalysis.Analysis);
                    if (matches.Count > 0)
                    {
                        ...
                        foreach (var move in moves)
                        {
                            ...
                            boardAnalysis.Analysis = boardAnalysis.Analysis.Remove(matchBeginsAt, move.Length).Insert(matchBeginsAt, lastMove);
                            matchBeginsAt += lastMove.Length + 1; // include the space
                        }
                    }
                }
            }
            m_repository.AddAnalysis(gameId, moveNumber, boardAnalysis);
        }
