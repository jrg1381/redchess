...
namespace EngineTests
{
	...
	internal class FenTests : AbstractChessTest
	{
		...
		public void StartPosition()
		{
			...
			Assert.AreEqual("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -", fen, "Fen representation of start position not as expected");
		}

		[Test]
		public void TwoKingsPosition()
		{
			m_emptyBoard.AddPiece(PieceFactory.CreatePiece(PieceType.WhiteKing,Location.C7));
			m_emptyBoard.AddPiece(PieceFactory.CreatePiece(PieceType.BlackKing, Location.A1));

			string fen = new Fen(m_emptyBoard, new CastlingRules()).ToFen();
			// Castling settings are invalid here, ignore this
			Assert.AreEqual("8/2K5/8/8/8/8/8/k7 w KQkq -", fen, "Fen representation of start position not as expected");
		}
		...
		public void NoCastling()
		{
			...
			// Castling settings are invalid here, ignore this
			Assert.AreEqual("rnbq1bnr/ppppkppp/4p3/8/8/4P3/PPPPKPPP/RNBQ1BNR w - -", fen, "Fen with no castling allowed not as expected");
		}
		...
		public void WithEnPassantPosition()
		{
			...
			string fen = new Fen(m_normalBoard, new CastlingRules()).ToFen();
			Assert.AreEqual("rnbqkbnr/pppppppp/8/8/P7/8/1PPPPPPP/RNBQKBNR b KQkq A3", fen, "Fen representation of start position not as expected");
		}
	}
}
