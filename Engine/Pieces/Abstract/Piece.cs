...
namespace Redchess.Engine.Pieces.Abstract
{
    internal abstract class Piece : IPiece
    {
        ...
        protected internal IEnumerable<Location> ReachableSquares(IList<int> dxSet, IList<int> dySet, IBoardExtended game)
        {
            ...
            for (int i = 0; i < dxSet.Count; i++)
            {
                Square generatedSquare = Position;
                while (!generatedSquare.Equals(Square.InvalidSquare))
                {
                    generatedSquare = new Square(generatedSquare.X + dxSet[i], generatedSquare.Y + dySet[i]);
